...
import { shortUppercaseId, DateFormat } from "./Notfications/Empty";
  ...
  return (
    <>
      <div className="overflow-x-auto overflow-hidden relative w-full">
        <div className="max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-subMain scrollbar-track-main hover:scrollbar-thumb-subMain">
      ...
      {/* Pagination */}
      {totalPages > 1 && (
        <div className="w-full flex-rows gap-6 mt-8">
          <button
            onClick={prevPage}
            disabled={currentPage === 1}
            className={`${sameClass} ${currentPage === 1 ? "opacity-50 cursor-not-allowed" : ""}`}
          >
          ...
          <button
            ...
            className={`${sameClass} ${currentPage === totalPages ? "opacity-50 cursor-not-allowed" : ""}`}
          >
