...
function AddMovie() {
  ...
  // Clear casts when the component mounts
  useEffect(() => {
...
//onSubmit
  ...
  //useEffect
  useEffect(() => {
    //if modal is false, reset the cast
    if (modalOpen === false) {
      setCast();
    }
    //if its success then reset the form and navigate to add movie
    if (isSuccess) {
      reset({
        name: "",
        time: 0,
    ...
    //if its error then show the error
    if (isError) {
      toast.error(isError);
      dispatch({type: "CREATE_MOVIE_RESET"});
    }
